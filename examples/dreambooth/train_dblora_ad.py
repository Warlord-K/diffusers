...
from transformers import UMT5EncoderModel, LlamaTokenizer, PretrainedConfig
...
validation_prompts = ["gorkem in a suit", "gorkem on a beach", "close up of gorkem", 'gorkem wearing a red shirt with "fal" written on it']
...
def save_model_card(
    ...
    if images is not None:
        for i, image in enumerate(images):
            ...
            widget_dict.append(
                {"text": validation_prompts[i], "output": {"url": f"image_{i}.png"}}
            )

    model_description = f"""
# AuraDiffusion DreamBooth LoRA - {repo_id}
...
import torch
pipeline = AutoPipelineForText2Image.from_pretrained('sAuraDiffusion/auradiffusion-v0.1a0', torch_dtype=torch.float16).to('cuda')
...
def import_model_class_from_model_name_or_path(
    ...
    if model_class == "UMT5EncoderModel":
...
def tokenize_prompt(tokenizer, prompt, max_length=256):
...
def main(args):
    ...
    if args.train_text_encoder:
        text_lora_parameters_one = list(filter(lambda p: p.requires_grad, text_encoder.parameters()))
        ...
        def compute_text_embeddings(prompt, text_encoder, tokenizer):
            with torch.no_grad():
                prompt_embeds, _ = encode_prompt(
                    text_encoder, tokenizer, prompt, args.max_sequence_length
                )
    ...
    if not args.train_text_encoder and not train_dataset.custom_instance_prompts:
        instance_prompt_hidden_states = compute_text_embeddings(
            args.instance_prompt, text_encoder, tokenizer
        )
    ...
    if args.with_prior_preservation:
        if not args.train_text_encoder:
            class_prompt_hidden_states = compute_text_embeddings(
                args.class_prompt, text_encoder, tokenizer
            )
    ...
    if accelerator.is_main_process:
        tracker_name = "dreambooth-auradiff-lora"
